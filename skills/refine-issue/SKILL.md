...
1. **Product ambiguity.** Identify decisions the Issue leaves open that would change what gets
   built, and ask about those with a recommendation for each. Do not ask what the repository or the
   brief already answers, or about reversible implementation details.
2. **Acceptance criteria.** Write criteria a reviewer can check against a diff: observable
   behaviour, not implementation steps. They are the contract implementation and review work from.
3. **Design checkpoint.** Decide whether the Story involves significant visual or interaction
   ...
   direction — and produce one concrete proposal consistent with that direction. When Pencil tooling
   is available and useful, a Pencil file is the preferred reference: use `plus-ultra:pencil-design`,
   which owns the design handoff, and save it at `designs/<issue>-<slug>.pen`. A mockup, screenshot, or
   described reference is the fallback when Pencil is unavailable or not useful. One explicit human
   approval may cover both the displayed Issue proposal and the concrete design when presented
   together; record the approved reference in the Issue. Feature-level design never touches
   `DESIGN.md`.
...
edits and report the Issue URL. GitHub's edit history preserves the intake.

Finish with one readiness statement: **refined and approved** once material product and design
decisions are settled; **ready for implementation** once its dependencies and any approved design
are also durably reachable through Git; **refined but blocked** otherwise, naming the blocker. Hand
a ready Story to `plus-ultra:implement-issue`; hand a refined Epic to decomposition instead.
